...
module CppTypeImpl implements LanguageTypeSig {
  ...
  class TypeDomainType = Cpp::TypeDomain;

  class RealDomainType = Cpp::RealDomain;

  class ComplexDomainType = Cpp::ComplexDomain;

  class ImaginaryDomainType = Cpp::ImaginaryDomain;
   ...
   */
  additional class CppType extends TCppType {
   ...
   */
  CppType getUnknownType() { result instanceof CppUnknownType }

  /**
   * Gets the `CppType` that represents a prvalue of type `void`.
   */
  CppType getVoidType() {
    exists(VoidType voidType | result.(CppPRValueType).hasType(voidType, false))
  }
   ...
   */
  CppType getTypeForGLValue(Type type) { result.(CppGLValueAddressType).hasType(type, true) }
  ...
  CppType getIntType() {
    exists(IntType type |
      ...
      result.(CppPRValueType).hasType(type, false)
    )
  }
   ...
   */
  CppType getBoolType() { exists(BoolType type | result.(CppPRValueType).hasType(type, false)) }
   ...
   */
  CppType getFunctionGLValueType() { result instanceof CppFunctionGLValueType }

  /**
   * Gets the `CppType` that represents a opaque of unknown type with size `byteSize`.
   */
  CppType getUnknownOpaqueType(int byteSize) {
    result.(CppUnknownOpaqueType).getByteSize() = byteSize
  }
   ...
   * `byteSize`.
   */
  CppType getCanonicalBooleanType(int byteSize) {
    exists(BoolType type |
      result.(CppWrappedType) = TPRValueType(type) and byteSize = type.getSize()
    )
  }
   ...
   * `byteSize`.
   */
  CppType getCanonicalSignedIntegerType(int byteSize) {
    result.(CppPRValueType) =
   ...
   * `byteSize`.
   */
  CppType getCanonicalUnsignedIntegerType(int byteSize) {
    result.(CppPRValueType) =
      TPRValueType(max(IntegralType type |
   ...
   */
  CppType getCanonicalFloatingPointType(int byteSize, int base, TypeDomain domain) {
    result.(CppPRValueType) =
   ...
   * `byteSize`.
   */
  CppType getCanonicalAddressType(int byteSize) {
    // We just use `NullPointerType`, since it should be unique.
    exists(NullPointerType type |
      type.getSize() = byteSize and
      result.(CppPRValueType) = TPRValueType(type)
    )
  }
   ...
   * `byteSize`.
   */
  CppType getCanonicalFunctionAddressType(int byteSize) {
    result.(CppFunctionGLValueType).getByteSize() = byteSize
  }

  /**
   * Gets the `CppType` that is the canonical type for `IRErrorType`.
   */
  CppType getCanonicalErrorType() {
    result.(CppPRValueType) = TPRValueType(any(ErroneousType type))
  }

  /**
   * Gets the `CppType` that is the canonical type for `IRUnknownType`.
   */
  CppType getCanonicalUnknownType() { result instanceof CppUnknownType }

  /**
   * Gets the `CppType` that is the canonical type for `IRVoidType`.
   */
  CppType getCanonicalVoidType() { result.(CppPRValueType) = TPRValueType(any(VoidType type)) }
