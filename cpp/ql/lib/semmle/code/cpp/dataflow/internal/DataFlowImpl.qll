...
signature module NoStateConfigSig {
  /**
   ...
   */
  default int explorationLimit() { none() }
...
private signature module GeneralConfigSig {
  /**
   * Holds if `source` is a relevant data flow source.
   */
  predicate isSource(Node source);
...
module NoStateImpl<NoStateConfigSig Config> {
  private module GeneralConfig implements GeneralConfigSig {
    predicate isSource(Node source) { Config::isSource(source) }

    predicate isSource(Node source, FlowState state) { none() }

    predicate isSink(Node sink) { Config::isSink(sink) }

    predicate isSink(Node sink, FlowState state) { none() }

    predicate isBarrier(Node node) { Config::isBarrier(node) }

    predicate isBarrier(Node node, FlowState state) { none() }

    predicate isBarrierIn(Node node) { Config::isBarrierIn(node) }

    predicate isBarrierOut(Node node) { Config::isBarrierOut(node) }

    predicate isAdditionalFlowStep(Node node1, Node node2) { Config::isAdditionalFlowStep(node1, node2) }

    predicate isAdditionalFlowStep(Node node1, FlowState state1, Node node2, FlowState state2) { none() }

    predicate allowImplicitRead(Node node, ContentSet c) { Config::allowImplicitRead(node, c) }

    int fieldFlowBranchLimit() { result = Config::fieldFlowBranchLimit() }

    FlowFeature getAFeature() { result = Config::getAFeature() }

    predicate sourceGrouping(Node source, string sourceGroup) { Config::sourceGrouping(source, sourceGroup) }

    predicate sinkGrouping(Node sink, string sinkGroup) { Config::sinkGrouping(sink, sinkGroup) }

    int explorationLimit() { result = Config::explorationLimit() }

    predicate includeHiddenNodes() { Config::includeHiddenNodes() }
  }

  import Impl<GeneralConfig>
}

private module Impl<GeneralConfigSig Config> {
  /**
  ...
  class Configuration extends string {
     ...
     * `state`.
     */
    predicate isSource(Node source, FlowState state) { Config::isSource(source, state) }
     ...
     * Holds if `sink` is a relevant data flow sink accepting `state`.
     */
    predicate isSink(Node sink, FlowState state) { Config::isSink(sink, state) }
     ...
     * `state`.
     */
    predicate isBarrier(Node node, FlowState state) { Config::isBarrier(node, state) }
