...
abstract class TranslatedElement extends TTranslatedElement {
   ...
   */
  abstract InstructionDesc getChildSuccessorDesc(TranslatedElement child);

  final Instruction getChildSuccessor(TranslatedElement child) {
    exists(InstructionTag tag |
      getChildSuccessorDesc(child) = SelfInstruction(tag) and
      result = getInstruction(tag)
    )
    or
    exists(TranslatedElement e |
