...
private predicate instructionTaintStep(Instruction i1, Instruction i2) {
  ...
  or
  i2 = any(LoadInstruction load |
    load.getSourceValueOperand().getAnyDef() = i1
  )
}
...
from DataFlow::Node source, DataFlow::Node sink, DefaultTaintTrackingCfg cfg
where cfg.hasFlow(source, sink)
select source.asInstruction(), sink.asInstruction()